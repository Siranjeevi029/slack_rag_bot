...
def _format_thread(client, messages: list) -> str:
    ...
    for m in messages:
        if m.get("subtype"):
            continue
        body = _clean(m.get("text", ""))
        if body:
            lines.append(f"{_name(client, m.get('user', ''))}: {body}")
    return "\n".join(lines)


def _reply(text: str, thread_text: str | None = None) -> str:
    q = _clean(text)
...
def _reply_in_thread(client, channel: str, thread_ts: str, text: str, say):
    ...
    say(text=_reply(text, thread_text=thread_text), thread_ts=thread_ts)
...
def on_mention(event, say, client):
    ...
    if thread_ts:
        ...
        say(text=_reply(event.get("text", "")), thread_ts=event["ts"])
